...
const secretMarkers = ['sk-', 'API_KEY=', 'BEGIN PRIVATE KEY', 'AWS_SECRET', 'ANTHROPIC_API_KEY', 'OPENAI_API_KEY'];
const ignoredDirs = new Set(['.git', '.next', 'node_modules', '__pycache__', '.pytest_cache']);
const ignoredFiles = new Set(['package-lock.json', 'security_scanner.py', 'verifier.py', 'health.mjs']);
...
function scanSecrets(root) {
  ...
  for (const path of walkFiles(root)) {
    if (ignoredFiles.has(path.split('/').pop())) continue;
...
export function buildHealthPayload(options = {}) {
  ...
  const productionGateKeys = [
    ...
    'no_secrets_in_repo',
    'no_paid_adapters_enabled',
    'no_spend_policy_active',
    'approval_gate_active',
    'audit_log_active',
    'backups_tested',
    'restore_tested',
    'uptime_monitor_active',
    'https_active',
    'firewall_active',
    'mobile_smoke_test_passes',
    'seo_metadata_present',
    'sitemap_present',
    'rollback_documented',
    'stale_source_scan_passes'
  ];
  const failedGates = productionGateKeys.filter((key) => yamlBoolean(config, 'production_gate', key) !== true);
  ...
  const checks = {
    ...
    ci: status(
      workflow.includes('pip install -r requirements.txt') &&
        workflow.includes('npm run validate:yaml') &&
        workflow.includes('npm run test:unit')
    ),
    registry: status(
      agentsRegistry.includes('id: local-research') &&
        agentsRegistry.includes('allowed_connectors: [playwright_local, local_files]') &&
    ...
    productionGate: status(failedGates.length === 0),
  ...
  if (failedGates.length) blockers.push(`production gates failing: ${failedGates.join(', ')}`);
