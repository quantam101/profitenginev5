...
export default function ProofOfWork() {
  ...
  useEffect(() => {
    ...
    getDistillationStats().then(setDist).catch(() => {});
  ...
  return (
    <section
      ...
      data-testid="proof-of-work-section"
    >
      <div className="mx-auto max-w-7xl px-6 md:px-10">
        ...
        <div className="mt-12 grid grid-cols-2 gap-px bg-line md:grid-cols-4">
          ...
          />
          <Stat
            label="cycles completed · 24h"
            value={pow ? pow.passed_cycles_24h : "—"}
            testId="pow-cycles"
          />
          <Stat
            label="signed assets · 24h"
            value={pow ? pow.signed_assets_24h : "—"}
            testId="pow-assets"
          />
          <Stat
            label="guard blocks · 24h"
            value={pow ? pow.guard_blocks_24h : "—"}
            testId="pow-guards"
          />
        </div>
        ...
        <div className="mt-12 grid grid-cols-1 gap-6 md:grid-cols-3">
          ...
          <div className="border border-line bg-bg-panel/60 p-8" data-testid="proof-distillation">
            ...
            <div className="mt-2 text-xs text-ink-muted">
              token cost saved vs all-Claude baseline
            </div>
            <div className="mt-6 space-y-1.5 text-[11px] text-ink-muted">
              ...
              <div>cache hit rate · <span className="text-ok">{dist?.cache_hit_rate != null ? `${Math.round(dist.cache_hit_rate * 100)}%` : "—"}</span></div>
            </div>
          </div>
        ...
        <div className="mt-10 flex flex-wrap items-center gap-3 text-[11px] text-ink-muted">
          ...
          live · auto-refreshed each visit · open <code className="font-mono text-ink">/api/proof-of-work</code> and <code className="font-mono text-ink">/api/distillation/stats</code> to verify
        </div>
      </div>
    </section>
  );
