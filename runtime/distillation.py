...
from services.distillation_service import CACHE_TTL_SECONDS, cache_clear, cache_stats, distill_text, estimate_tokens, to_yaml_payload
...
_BOILERPLATE_RE = re.compile('(cookie\\s+policy|privacy\\s+policy|terms\\s+of\\s+(use|service)|all\\s+rights\\s+reserved|copyright\\s+\\d{4}|subscribe\\s+to\\s+our\\s+newsletter|sign\\s+up|log\\s+in|advertisement|share\\s+this\\s+article)', re.I)
_STOP_WORDS = frozenset('a an the is are was were be been being have has had do does did will would could should may might shall can i you he she it we they what which who this that these those of in on at by for with from to as or and but if so not no nor yet both either neither than then such than because since while though although when where how'.split())

def estimate_tokens(text: str) -> int:
    """Rough token estimate: 1 token ≈ 4 chars for English prose."""
    return max(1, len(text) // 4)

def distill(text: str, objective: str='', char_budget: int=_BUDGET_DEFAULT, run_dedup: bool=True, run_focus: bool=True) -> str:
    """
...
def keyword_focus(text: str, objective: str, top_n: int=_FOCUS_TOP_N) -> str:
    ...
    """
    obj_words = Counter((w for w in re.findall('[a-z0-9]+', objective.lower()) if w not in _STOP_WORDS and len(w) > 2))
    if not obj_words:
        return text
    sentences = _split_sentences(text)
...
def _split_sentences(text: str) -> List[str]:
    ...
    """
    text = re.sub('(\\b(?:Mr|Mrs|Ms|Dr|Prof|Sr|Jr|vs|etc|e\\.g|i\\.e|al))\\.', '\\1<DOT>', text)
...
def distill_prompt(system: str, user: str, objective: str, system_budget: Optional[int]=None, user_budget: Optional[int]=None) -> tuple[str, str, dict]:
    """
    ...
    metrics = {'system_before': sys_before, 'system_after': len(comp_system), 'user_before': user_before, 'user_after': len(comp_user), 'total_before_tokens': estimate_tokens(system + user), 'total_after_tokens': estimate_tokens(comp_system + comp_user), 'reduction_pct': round(100 * (1 - (len(comp_system) + len(comp_user)) / max(1, sys_before + user_before)), 1)}
...
@router.get('/config')
async def config():
    return {'ttl_seconds': CACHE_TTL_SECONDS, 'token_cost_per_1k_usd': float(os.environ.get('TOKEN_COST_PER_1K', '0.0001')), 'compression_enabled': True, 'yaml_payloads_enabled': True, 'tiers': {'tier_1': 'Local rule-based — no LLM (hashtags, formatting, slugs)', 'tier_2': 'Distill + cache lookup — LLM only on miss', 'tier_3': 'LLM call with compressed input + YAML payloads'}}
...
async def preview(body: PreviewRequest):
    ...
    return {'original': {'chars': len(body.text or ''), 'tokens_est': estimate_tokens(body.text or '')}, 'distilled': {'text': distilled, 'chars': len(distilled), 'tokens_est': estimate_tokens(distilled)}, 'savings': {'chars': max(0, len(body.text or '') - len(distilled)), 'tokens_est': max(0, estimate_tokens(body.text or '') - estimate_tokens(distilled)), 'percent': round(100 * (1 - len(distilled) / max(1, len(body.text or ''))), 1) if body.text else 0.0}, 'yaml_payload': yaml_str, 'json_payload': json_alt, 'payload_savings_tokens_est': max(0, estimate_tokens(json_alt) - estimate_tokens(yaml_str))}
