...
def implemented_agent_ids() -> set[str]:
    ...
    return {path.stem.replace('_', '-') for path in impl_dir.glob('*.py') if path.name != '__init__.py'}
...
async def list_agents(status: str=None, db=Depends(get_db)):
    """List all agents"""
    query = {}
    if status:
        query['status'] = status
...
async def execute_agent(agent_id: str, db=Depends(get_db)):
    """Execute an agent action"""
    agent = await db.agents.find_one({'id': agent_id}, {'_id': 0})
    if not agent:
        raise HTTPException(status_code=404, detail='Agent not found')
    await db.agents.update_one({'id': agent_id}, {'$set': {'last_run': datetime.now(timezone.utc).isoformat(), 'updated_at': datetime.now(timezone.utc).isoformat()}, '$inc': {'run_count': 1}})
